...
export default function EditBannerForm({ banner }: { banner: Banner }) {
  ...
  const [previewMobileImage, setPreviewMobileImage] = useState(banner.mobileImage || '');
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await updateBannerAction(banner.id, formData);
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-8 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-heading font-bold text-[#0B1F3A]">Edit Banner</h1>
            <p className="text-slate-500 text-sm">Update banner details and scheduling</p>
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div>
                ...
                <input type="text" name="title" defaultValue={banner.title} required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:bg-white transition-all" />
              </div>

              <div>
                <label className="block text-xs font-bold tracking-widest uppercase text-slate-500 mb-2">Subtitle (Optional)</label>
                <input type="text" name="subtitle" defaultValue={banner.subtitle || ''} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:bg-white transition-all" />
              </div>

              <div>
                <label className="block text-xs font-bold tracking-widest uppercase text-slate-500 mb-2">Button Link (Optional)</label>
                <input type="text" name="link" defaultValue={banner.link || ''} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:bg-white transition-all" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold tracking-widest uppercase text-slate-500 mb-2">Start Date (Optional)</label>
                  <input type="datetime-local" name="startDate" defaultValue={formatDateForInput(banner.startDate)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:bg-white transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-bold tracking-widest uppercase text-slate-500 mb-2">End Date (Optional)</label>
                  <input type="datetime-local" name="endDate" defaultValue={formatDateForInput(banner.endDate)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:bg-white transition-all" />
                </div>
              </div>
            </div>
