...
export default function NewGalleryImagePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-heading font-bold text-[#0B1F3A]">Add Gallery Image</h1>
        <p className="text-slate-500 text-sm mt-1">Upload a new image to your gallery.</p>
      </div>
        ...
        <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div>
                <label className="block text-xs font-bold tracking-widest uppercase text-slate-500 mb-2">Image File</label>
                <input 
                  type="file" 
                  name="imageFile" 
                  accept="image/*"
            ...
            <div className="aspect-square rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 flex items-center justify-center overflow-hidden relative">
              {previewImage ? (
          ...
          <div className="mt-8 pt-8 border-t border-slate-100 flex justify-end gap-4">
            <Link href="/admin/gallery" className="px-6 py-3 rounded-full font-bold tracking-widest uppercase text-xs text-slate-500 hover:bg-slate-100 transition-colors">
              Cancel
            </Link>
            <button type="submit" disabled={loading} className="bg-[#0B1F3A] hover:bg-[#153460] text-white px-8 py-3 rounded-full font-bold tracking-widest uppercase text-xs transition-colors shadow-md flex items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed">
              {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Uploading...</> : <><Save className="w-4 h-4" /> Save Image</>}
            </button>
          </div>
        </div>
      </form>
